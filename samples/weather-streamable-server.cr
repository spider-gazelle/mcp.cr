...
class StreamableServer
  ...
  def self.run(port = 8080)
    ...
    puts "Streamable HTTP Server listening on http://localhost:#{port}/mcp"
    server.listen(port)
  end
  ...
  private def self.handle_post_rpc(context)
  ...
  private def self.handle_sse_stream(context)
    session_id = context.request.headers[MCP::Server::StreamableHttpServerTransport::MCP_SESSION_ID]?
  ...
  private def self.handle_delete_session(context)
    session_id = context.request.headers[MCP::Server::StreamableHttpServerTransport::MCP_SESSION_ID]?
